...
FONT_FILES = {
    "BigShouldersDisplay[wght].ttf": "ofl/bigshouldersdisplay/BigShouldersDisplay%5Bwght%5D.ttf",
...
SECTIONS = {
    "whoami": "whoami",
    "work": "ls ~/case-files",
    "principles": "cat protocols.md",
    "activity": "./patrol-log.sh",
    "contact": "./bat-signal.sh",
}
BUTTONS = (("portfolio", "Portfolio"), ("linkedin", "LinkedIn"), ("email", "Email"), ("resume", "Résumé"))
...
def load_fonts() -> banner.Fonts:
    ...
    return banner.Fonts(
        display=ref("BigShouldersDisplay[wght].ttf", ("wght", 600)),
        display_bold=ref("BigShouldersDisplay[wght].ttf", ("wght", 800)),
...
def readme() -> str:
    tiles = [
        f'<a href="{p.link}">{themed(f"project-{p.slug}", f"{p.name}: {p.kind}", "49%")}</a>'
        for p in PROJECTS
    ]
    ...
    sources = " · ".join(f'<a href="{url}">{name}</a>' for name, url in SOURCES)
    buttons = " ".join(
...
<a href="{LINKS["portfolio"]}"><img src="./assets/banner.svg" width="100%" alt="Harshith Pramod. Full-stack Developer. The bat-signal over a rainy Gotham skyline."></a>

<br><br>

{themed("header-whoami", "$ whoami")}

{themed("hollow", "ASCII art: a Hollow mask from Bleach", "49%")} {themed("whoami", "Role, stack, and what I have shipped", "49%")}

<br><br>

{themed("header-work", "$ ls ~/case-files")}

{projects}

<sub>Source: {sources}</sub>

<br><br>

{themed("header-principles", "$ cat protocols.md")}

{themed("principles", "How I build: evidence over suspicion; lock it at the source; AI earns its place")}

<br><br>

{themed("header-activity", "$ ./patrol-log.sh")}

{snake}

<br><br>

{themed("header-contact", "$ ./bat-signal.sh")}
...
def main() -> None:
    ...
    mask, tint = hollow.render()
    art = ascii_card.Art(mask, tint, "~/hollowfication", "hollow mask · bleach", "ASCII art: a Hollow mask from Bleach")
    for theme in THEMES:
        suffix = f"-{theme.name}.svg"
        write("hollow" + suffix, ascii_card.render(theme, art))
